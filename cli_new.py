...
class CommandListener(object):
    ...
    def __init__(self):
        ...
        #user parsers
        user_create = user_subparsers.add_parser('register', parent=[get_name])
        user_create.add_argument('--password', '--pass')
        user_create.add_argument('--admin', action="store_true")
        user_del = user_subparsers.add_parser('delete', parent=[get_name])
        user_parser.add_argument('--project', '--proj')
        user_con = user_subparsers.add_parser('connect', parent=[get_name])
        user_discon = user_subparsers.add_parser('disconnect', parent=[get_name])
        
        #project parsers
        proj_list = project_subparsers.add_parser('list', parents=[get_name])
        proj_list_group = proj_list.add_mutually_exclusive_group()
        proj_list_group.add_argument('--headnode', '--hnode')
        proj_list_group.add_argument('--network', '--net')
        proj_list_group.add_argument('--nodes', '--node')
        proj_dis = project_subparsers.add_parser('disconnect', parent=[get_name])
        proj_dis_group = proj_dis.add_mutually_exclusive_group(required=True)
        proj_dis_group.add_argument('--node')
        proj_dis_group.add_argument('--network')
        proj_dis_group.add_arguments('--user')
        proj_del = project_subparsers.add_parser('delete', parent=[get_name])
        proj_create = project_subparsers.add_parser('register', parent=[get_name])
        proj_connect = project_subparsers.add_parser('connect', oarent=[get_name])
        proj_con_group = proj_connect.add_mutually_exclusive_group(required=True)
        proj_con_group.add_argument('--node')
        proj_con_group.add_argument('--network')
        proj_con_group.add_arguments('--user')
        
        #switch parsers
        switch_reg = switch_subparsers.add_parser('register')
        switch_reg_subtype = switch_reg.add_mutually_exclusive_group(required=True)
        switch_reg_subtype.add_argument('--mock', nargs=3, metavar=('host','user','password') )
        switch_reg_subtype.add_argument('--impi', nargs=3, metavar=('host','user','password') )
        switch_del = switch_subparsers.add_parser('delete', parents=[get_name])
        switch_list = switch_subparsers.add_parser('list')
        switch_show = switch_subparsers.add_argument('show', parents=[get_name])
